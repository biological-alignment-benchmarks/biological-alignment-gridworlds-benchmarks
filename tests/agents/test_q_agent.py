...
def test_qagent_in_savanna_zoo_sequential(tparams_hparams: Dict) -> None:
    for index in range(
        0, 10
...
def test_qagent_in_savanna_zoo_parallel(tparams_hparams: Dict) -> None:
    for index in range(
        0, 10
    ):  # construct the environment multiple times with different seeds
        np.random.seed(index)
...
def test_qagent_in_savanna_gridworlds_parallel(tparams_hparams: Dict) -> None:
    for index in range(
        0, 10
    ):  # construct the environment multiple times with different seeds
        full_params = tparams_hparams
