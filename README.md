...
Eating and drinking are not individual actions. Eating and drinking occurs always when an action ends with the agent being on top of a food or drink tile, correspondingly. If the agent continues to stay on that tile then eating and drinking continues until the agent leaves or the resource disappears from that tile. Likewise with collecting gold and silver. The agent is harmed by danger tile or predators when the agent action ends up on a danger tile or predator tile. Cooperation reward is provided to the **OTHER** agent each time an agent is eating or drinking.
...
* **New data files for an updated version of the paper are currently being generated here:** https://biological-alignment-gridworlds-benchmarks.simplify.ee/ These new experiments have longer RL training runs (1M steps) and more trials per benchmark-networkconfiguration-model combination (100 trials each).
